    ...
    def sense(self):
        ...
        for i, landmark in enumerate(self.landmarks):
            ...
            if abs(dx)<=self.measurement_range and abs(dy)<=self.measurement_range: 
                # I did not include previous condition because
                # adding both values from above 
                # abs(dx) + abs(dy) <= 2*self.measurement_range
                # squaring on both sides
                # (abs(dx) + abs(dy))**2 <= 4*self.measurement_Range**2 - eqn 1
                # but (abs(dx))**2 + abs(dy)**2 <= (abs(x) + abs(y))**2 - eqn 2 Equality holds when values are zero
                # Using eqn 2 in eqn 1
                # abs(dx)**2 + abs(dy)**2 <= 4*self.measurement_range
                # So I am ignoring previous conidition. 
